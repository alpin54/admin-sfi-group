...
const ModalCollectionView = ({
    ...
    try {
      const payload = {
        ...
        ...(methodType === 'post'
          ...
          : {
              status: values.status ?? initialValues?.status ?? true,
              updated_by: user?.id
            })
      };
  ...
  const footerComponent = (
    <>
      ...
      <Button type='primary' htmlType='submit' form='form-color' loading={loading}>
        Save
      </Button>
    </>
  );
  ...
  return (
    <>
      <Modal
        ...
        <Form form={formInstance} id='form-color' layout='vertical' onFinish={handleFinish} autoComplete='off'>
          <Form.Item name='id' hidden>
            <Input />
          </Form.Item>

          <Form.Item name='name' label='Color Name' rules={[{ required: true, message: 'Please enter Color name' }]}>
            <Input />
          </Form.Item>
          <Form.Item
            name='code'
...
export default ModalCollectionView;
