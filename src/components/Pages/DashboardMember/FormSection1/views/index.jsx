...
const FormDashboardSection1View = (props) => {
  ...
  useEffect(() => {
    if (data) {
      const mappedFields = {
        ...
        en: {
          title: data?.title?.en || '',
          textButton: data?.textButton?.en || ''
        },
        id: {
          title: data?.title?.id || '',
          textButton: data?.textButton?.id || ''
        }
      };
  ...
  return (
    <Card loading={ready}>
      <SectionHeader
        title='Waiting for Dealer Account Approval'
        publish={data?.status}
        onPublish={() => handlePublish(data)}
      />
      <Form
        form={formInstance}
        id='form-dashboard-section1'
        layout='vertical'
        ...
        <TranslationTabs>
          {(lang) => (
            <>
              ...
              <Form.Item
                name={[lang, 'textButton']}
                label='Text Button'
                rules={[{ required: true, message: 'Text Button is required' }]}>
          ...
          <Space size={16}>
            {isEdit ? (
              <>
                ...
                <Button type='primary' htmlType='submit' form='form-dashboard-section1' loading={loading}>
                  Save
                </Button>
              </>
            ) : (
...
export default FormDashboardSection1View;
