...
const FormDashboardMemberSection2View = (props) => {
    ...
    if (data) {
      const mappedFields = {
        en: {
          ...
          button: data?.button?.en || '',
          buttonText: data?.buttonText?.en || '',
          whatsAppNumber: data?.whatsAppNumber?.en || ''
        },
        id: {
          title: data?.title?.id || '',
          description: data?.description?.id || '',
          button: data?.button?.id || '',
          buttonText: data?.buttonText?.id || '',
          whatsAppNumber: data?.whatsAppNumber?.id || ''
        }
      };
  ...
  return (
    <Card loading={ready}>
      <SectionHeader title='Join as a Dealer' publish={data?.status} onPublish={() => handlePublish(data)} />
      <Form
        form={formInstance}
        id='form-about-us-section2'
        layout='vertical'
          ...
          {(lang) => (
            <>
              ...
              <Form.Item
                name={[lang, 'button']}
                label='Button'
                rules={[{ required: true, message: 'Button is required' }]}>
                <Input allowClear readOnly={!isEdit} />
              </Form.Item>
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    name={[lang, 'buttonText']}
                    label='Text Button'
          ...
          <Space size={16}>
            {isEdit ? (
              <>
                ...
                <Button type='primary' htmlType='submit' form='form-about-us-section2' loading={loading}>
                  Save
                </Button>
              </>
            ) : (
...
export default FormDashboardMemberSection2View;
