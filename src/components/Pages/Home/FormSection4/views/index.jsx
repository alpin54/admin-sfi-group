...
const FormHomeSection4View = (props) => {
  ...
  return (
    <Card loading={ready}>
      <SectionHeader title='Image' publish={data?.status} onPublish={() => handlePublish(data)} />
      <Form form={formInstance} id='form-section-image' layout='vertical' onFinish={handleFinish} autoComplete='off'>
        <Form.Item name='id' hidden>
          <Input />
        </Form.Item>
        <Form.Item name='image' label='Image' valuePropName='file' getValueFromEvent={(e) => e} help='1440px x 704px'>
          <UploadImage value={{ url: data?.image }} disabled={!isEdit} />
        </Form.Item>
        <Form.Item name='title' label='Alt' rules={[{ required: true, message: 'Alt is required' }]}>
          ...
          <Space size={16}>
            {isEdit ? (
              <>
                ...
                <Button type='primary' htmlType='submit' form='form-section-image' loading={loading}>
                  Save
                </Button>
              </>
            ) : (
...
export default FormHomeSection4View;
