...
const FormCheckoutSection1View = (props) => {
    ...
    if (data) {
      const mappedFields = {
        en: {
          ...
          optionTitle: data?.optionTitle?.en || ''
        },
        id: {
          title: data?.title?.id || '',
          description: data?.description?.id || '',
          optionTitle: data?.optionTitle?.id || ''
        }
      };
  ...
  return (
    <Card loading={ready}>
      <Form form={formInstance} id='form-checkout' layout='vertical' onFinish={handleFinish} autoComplete='off'>
          ...
          <Space size={16}>
            {isEdit ? (
              <>
                ...
                <Button type='primary' htmlType='submit' form='form-checkout' loading={loading}>
                  Save
                </Button>
              </>
            ) : (
...
export default FormCheckoutSection1View;
