...
const FormContactUsSection2View = (props) => {
  ...
  useEffect(() => {
    if (data) {
      formInstance?.setFieldsValue({
        ...data,
        en: { title: data?.title?.en || '', address: data?.address?.en || '' },
        id: { title: data?.title?.id || '', address: data?.address?.id || '' }
      });
    }
  }, [data, formInstance]);
  ...
  return (
    <Card loading={ready}>
      <SectionHeader title='Address' publish={data?.status} onPublish={() => handlePublish(data)} />
      <Form
        form={formInstance}
        id='form-contact-us-section2'
        layout='vertical'
          ...
          <Space size={16}>
            {isEdit ? (
              <>
                ...
                <Button type='primary' htmlType='submit' form='form-contact-us-section2' loading={loading}>
                  Save
                </Button>
              </>
            ) : (
