...
const FormRewardView = (props) => {
  ...
  const handleFinish = async (values) => {
    try {
      ...
      if (response && response.data) {
        ...
        // router.push('/pages/member-rewards');
      } else {
        notify({
          type: 'error',
  ...
  return (
    <Card>
      <Form form={formInstance} id='form-member-rewards' layout='vertical' onFinish={handleFinish} autoComplete='off'>
        ...
        <Form.Item>
          <Space size={16}>
            {isEdit ? (
              <Button type='primary' htmlType='submit' form='form-member-rewards' loading={loading}>
                Save
              </Button>
            ) : (
