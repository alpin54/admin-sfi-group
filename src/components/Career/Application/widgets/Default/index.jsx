...
// -- data
import dataDummy from '@components/Career/Application/data';
...
const CareerApplicationWidget = ({ slug }) => {
  ...
  const dataDummyNormalize = useMemo(() => {
    ...
    const summary = Array.isArray(dataDummy?.summary) ? dataDummy.summary : dataDummy?.summary;
    return {
      summary,
      data: { list, total: list.length },
      jobTypes: Array.isArray(dataDummy?.jobTypes) ? dataDummy.jobTypes : [],
      workplaces: Array.isArray(dataDummy?.workplaces) ? dataDummy.workplaces : []
    };
  }, []);
  ...
  const normalizedData = toNormalized(dataDummyNormalize?.data);
  ...
  const filterByCareerId = (payload, careerId) => {
    ...
    if (Array.isArray(payload)) {
      const anyHasCareerId = payload.some((r) => getCareerId(r) !== undefined && getCareerId(r) !== null);
      if (!anyHasCareerId) return payload;
      return payload.filter((r) => String(getCareerId(r)) === idStr);
    }

    if (payload && Array.isArray(payload.list)) {
      const anyHasCareerId = payload.list.some((r) => getCareerId(r) !== undefined && getCareerId(r) !== null);
      if (!anyHasCareerId) return payload;
  ...
  return (
    <CareerApplicationView
      summary={dataDummyNormalize?.summary}
      data={finalData}
      loading={false}
      ...
      categoryJobTypeOptions={dataDummyNormalize?.jobTypes}
      categoryWorkplaceOptions={dataDummyNormalize?.workplaces}
      // refetch={refetch}
    />
  );
};
...
/*
LEGACY (kept for reference, auto-formatted/mangled content):
...
... (see git history / previous editor buffer)
*/
